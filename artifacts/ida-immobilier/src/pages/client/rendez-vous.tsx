...
export default function RendezVous() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="font-serif text-3xl font-bold text-primary">Mes Rendez-vous</h1>
      ...
      {isLoading ? (
            ...
            <Card key={apt.id} className="overflow-hidden border-l-4 border-l-primary">
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row justify-between gap-6">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs font-semibold px-2 py-1 bg-muted uppercase tracking-wider rounded-sm">
                          {apt.type === 'visit' ? 'Visite' : apt.type === 'meeting' ? 'Rendez-vous' : 'Autre'}
                        </span>
                      </div>
                      <h3 className="font-serif text-xl font-bold mt-2">{apt.propertyTitle || "Rendez-vous agence"}</h3>
                    </div>
                  ...
                  <div className="bg-muted p-4 rounded-lg flex flex-col justify-center min-w-[200px]">
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-sm text-muted-foreground">Avec {apt.agentName || "votre agent"}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <div className="bg-card border border-border p-12 text-center rounded-xl">
          ...
          <p className="text-muted-foreground mb-4">Vous n'avez pas de visites ou de rendez-vous programmés.</p>
        </div>
      )}
    </div>
  );
